...
def deal_card():
    check = random.choice(cards)
    return check

#Hint 6: Create a function called calculate_score() that takes a List of cards as input and returns the score. 
#Look up the sum() function to help you do this.
'''Function to calculate score of all cards'''
def calculate_score(player_cards):
    # The parameter 'cards' represents each hand of cards for both user and computer
...
"""Function to compare scores and determine outcome"""
def compare(user_score, computer_score):
    if user_score == computer_score: # if both get Blackjack
        return "It's a draw."
...
def play_game():
    ...
    # Deal 2 cards twice for each player
    for i in range(0,2): 
        user_cards.append(deal_card())
        computer_cards.append(deal_card())

    #Hint 11: The score will need to be rechecked with every new card drawn and the checks in Hint 9 need to be repeated until the game ends.
    '''Determine if user should draw cards.'''
    is_game_over = False 
    while is_game_over == False:
        ...
        # User cannot draw cards if these conditions is met
        if user_score == 0 or computer_score == 0 or user_score > 21:
            is_game_over = True
            print("User cannot draw cards anymore.")

        #Hint 10: If the game has not ended, ask the user if they want to draw another card. If yes, then use the deal_card() function to add another card to the user_cards List. If no, then the game has ended.
        # User can draw cards
        else: 
            print("No one has won yet... Game continues!")
            ...
            if draw_another == 'y':
                ...
                is_game_over = True
                print("Player has decided not to draw cards. Computer's turn to draw cards.")
